'use client';
import { Button } from '@/core/adnui/components/ui/button';
import type { IUserState } from '@/lib/features/userSlice/type';
import { useAppSelector } from '@/lib/hooks/redux';
import { useSafeExecute } from '@/common/hooks/useSafeExecute';
import { useLazyGetListUserQuery } from '@/lib/services/modules';
import type { RootState } from '@/lib/services/store';
import Image from 'next/image';
import React from 'react';
import type { IAuthState } from '@/lib/features/authSlice/type';
import { useLogout } from '@/common/hooks';
import useAppNavigation from '@/common/hooks/useAppNavigation';

const ExampleClientComponent = () => {
  const safeExecute = useSafeExecute();
  const userState = useAppSelector<RootState, IUserState>((state) => state.user);
  const authState = useAppSelector<RootState, IAuthState>((state) => state.auth);
  const [triggerGetListUser] = useLazyGetListUserQuery();
  const {goToLogin} = useAppNavigation();
  const logout = useLogout();



  const handleClick = async () => {
    await safeExecute(() => {
      return triggerGetListUser().unwrap();
    },
      (data) => {
        console.log(data);
      },
      (error) => {
        console.log(error);
      });
  };

  return (
    <main className="flex min-h-screen flex-col bg-neutral-700 items-center justify-between p-24">
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm lg:flex">
        <p className="fixed left-0 top-0 flex w-full justify-center border-b border-gray-300 bg-gradient-to-b from-zinc-200 pb-6 pt-8 backdrop-blur-2xl dark:border-neutral-800 dark:bg-zinc-800/30 dark:from-inherit lg:static lg:w-auto  lg:rounded-xl lg:border lg:bg-gray-200 lg:p-4 lg:dark:bg-zinc-800/30">
          Welcome to&nbsp;
          <code className="font-mono font-bold">Next.js + Redux</code>&nbsp;
          starter kit (By&nbsp; Nguyễn Đình Át)
        </p>
        <div className="fixed bottom-0 left-0 flex h-48 w-full items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black lg:static lg:h-auto lg:w-auto lg:bg-none">
        </div>
      </div>
      <Button
        className="mt-6"
        onClick={handleClick}
      >
        Click me
      </Button>
      <Button
        className="mt-6"
        onClick={() => {
          logout();
          goToLogin();
        }}
      >
        Logout
      </Button>
      <div className="relative flex flex-col md:flex-row gap-6 place-items-center before:absolute before:h-[300px] before:w-full sm:before:w-[480px] before:-translate-x-1/2 before:rounded-full before:bg-gradient-radial before:from-white before:to-transparent before:blur-2xl before:content-[''] after:absolute after:-z-20 after:h-[180px] after:w-full sm:after:w-[240px] after:translate-x-1/3 after:bg-gradient-conic after:from-sky-200 after:via-blue-200 after:blur-2xl after:content-[''] before:dark:bg-gradient-to-br before:dark:from-transparent before:dark:to-blue-700 before:dark:opacity-10 after:dark:from-sky-900 after:dark:via-[#0141ff] after:dark:opacity-40 before:lg:h-[360px] z-[-1]">
        <Image
          className="relative dark:drop-shadow-[0_0_0.3rem_#ffffff70] dark:invert"
          src="/next.svg"
          alt="Next.js Logo"
          width={180}
          height={37}
          priority
        />
      </div>
    </main>
  );
};

export default ExampleClientComponent;


